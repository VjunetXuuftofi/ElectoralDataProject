...
```
## Introduction
For much of early American history, the Federal government allowed states to decide how their own elections would be held, even if those elections were to the Federal congress. This led to a number of interesting differences in how states conducted their elections. Some worked by first past the post systems; others established runoffs if no candidate won a majority, occasionally generating as many as seven elections for one seat. States held their elections at vastly different times, with elections for one Congress sometimes taking place over the course of a full year.

Additionally, states had the exclusive power to determine just who, exactly, would be able to elect each representative. Along with controlling voting eligiblity, they were allowed to district how they pleased. Many states, particularly smaller ones, chose to set up at-large elections. Georgia, for instance, in 1838 could elect 9 representatives to the U.S. House. It chose to set up a system whereby each party would nominate 9 candidates, each voter would cast 9 votes, and the top 9 candidates receiving the most votes would win.

In 1840, Congress inserted a controversial provision into the Apportionment Act. It mandated that all states with multi-member districts to elect U.S. Representatives must switch to a single-member system. A question that arises from this provision is clear: why, exactly, did Congress want to make such a change?

## Analysis

Martin H. Quitt hypothesized that because single-member districts tended to "sweep" a single party into every seat of the state's delegation, and because voters tend to "moderate" the majority party, the majority party (as represented in Congress) would be incentivized to mandate single-member districts to minimize losses.
...
ggplot(minority_power_summarised, aes(x=is_single_member, y=percentage_zero*100)) + geom_bar(stat="identity") +
  ...
  ggtitle("One Party Controlls All Seats More Often in States With Multi-Member Districts") + 
...
So the data provides evidence of Quitts "sweep effect". But does it provide evidence that this effect does, in fact, hurt the majority party in the next election?

```{r}
differences <- filtered %>% arrange(congress) %>%
  group_by(state, party) %>%
  mutate(is_majority = shift(actual, type="lag") > .5) %>%
  filter(is_majority == T) %>%
  mutate(difference = c(diff(actual), NA))

differences <- subset(differences, mean_elected_district >= 1 & num_elected_state > 1)
